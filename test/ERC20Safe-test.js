...
describe("ERC20Safe", async function () {
  ...
  describe("whitelistToken", async function () {
    ...
    it("emits event", async function () {
      await expect(safe.whitelistToken(afc.address, 1)).to.emit(safe, "TokenWhitelisted").withArgs(afc.address, 1);
    });
    ...
    describe("called by non admin", async function () {
      ...
      it("reverts", async function () {
        await expect(nonAdminSafe.whitelistToken(afc.address, 1)).to.be.revertedWith(
  ...
  describe("removeTokenFromWhitelist", async function () {
    ...
    describe("called by non admin", async function () {
      ...
      it("reverts", async function () {
        await expect(nonAdminSafe.removeTokenFromWhitelist(afc.address)).to.be.revertedWith(
          "Access Control: sender is not Admin",
        );
      });
    });
  ...
  describe("setBatchSize", async function () {
    ...
    it("emits event", async function () {
      await expect(safe.setBatchSize(newBatchSize)).to.emit(safe, "BatchSizeChanged").withArgs(newBatchSize);
    });
  ...
  describe("setBatchTimeLimit", async function () {
    ...
    describe("called by non admin", async function () {
      ...
      it("reverts", async function () {
        await expect(nonAdminSafe.setBatchTimeLimit(newBatchTimeLimit)).to.be.revertedWith(
          "Access Control: sender is not Admin",
        );
      });
    });
